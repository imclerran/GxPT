...
namespace Mcp35.Client.Transport
{
    ...
    internal sealed class StdioChannel : IDuplexMessageChannel
    {
        ...
        public void Dispose()
        {
            ...
            // Forceful (app/host shutdown): kill now and return. TerminateProcess guarantees the
            // child dies without us waiting, and the reader/stderr threads are background threads,
            // so we skip both WaitForExit and the joins. Effectively instantaneous.
            if (ForcefulShutdown)
            {
